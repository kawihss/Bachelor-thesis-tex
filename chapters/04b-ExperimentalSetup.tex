...
A fixed random seed was used across all models and experiments to support reproducibility (see Table~\ref{tab:hpo-config} in Appendix~\ref{appx:B}). Note, however, that this seed does not cover the  data augmentation applied during preprocessing, so all experiments must be run on the same, already-generated Parquet dataset rather than regenerating preprocessing independently.
 Each model received the  normalised feature set 
   % all normalised features it can process
    as a context window and outputs displacement increments $(\Delta x, \Delta y)$. The kinematic models additionally received x and y coordinates as an input, an input we omitted for the deep learning models as per Section \ref{subsec:featureengineering}. %to avoid learning specific trajectories instead of vessel dynamics. The kinematic models are incapable of "overfitting " to a specific location, so raw coords are fine
    ...
    %For testing purposes, both the tuning and evaluation stages support configurable subsampling of data files. %not necessary to know
...
\begin{table}[t]
    ...
    \caption[LSTM Hyperparameter Search Space]{Hyperparameter search space used for the \ac{LSTM} model optimization. Continuous parameters are sampled from uniform or log-uniform distributions, categorical parameters are selected from predefined sets. For the \texttt{feature\_set}, the search space consists of all feature combinations formed by the base input $\{\mathrm{d}x_{\mathrm{norm}}, \mathrm{d}y_{\mathrm{norm}}\}$ united with any subset $S$ drawn from the power set $\mathcal{P}(\mathcal{A})$. The $\sin$ and $\cos$ components of \acf{COG} ($\sin(\mathrm{cog})_{\mathrm{norm}}$ and $\cos(\mathrm{cog})_{\mathrm{norm}}$) are treated as an indivisible pair within $\mathcal{A}$.}
    \label{tab:lstm-hpo}
\end{table}
                          

\begin{comment} vlt so?  https://gemini.google.com/app/d39ceb4ec651a3ad
    \texttt{feature\_set}   & Categorical & $\{\{\texttt{dx\_norm},\, \texttt{dy\_norm}\} \cup S \mid S \in \mathcal{P}(\mathcal{A})\}$ \\
    \bottomrule
    \\[1ex]
    \small \textit{Note:} $\mathcal{A} = \{\{\texttt{cog\_sin\_norm}, \texttt{cog\_cos\_norm}\}, \texttt{sog\_norm}, \texttt{rot\_norm}, \texttt{dt\_norm}\}$
    \caption[LSTM Hyperparameter Search Space]{Hyperparameter search space used for the \ac{LSTM} model optimization. Continuous parameters are sampled from uniform or log-uniform distributions, categorical parameters are selected from predefined sets. For the \texttt{feature\_set}, the search space consists of all combinations formed by the base input $\{\texttt{dx\_norm}, \texttt{dy\_norm}\}$ combined with any subset $S$ drawn from the power set $\mathcal{P}(\mathcal{A})$, where the sine and cosine of \acf{COG} are treated as an indivisible pair.}
    \label{tab:lstm-hpo}
\end{comment}
...
%Eigentlich alles redundant mit Evaluation Framework
        ...
        Specifically, the last 4 nodes of the initial hidden state $h_0$ and cell state $c_0$ are set to the one-hot vector. The remaining nodes are initialised to zero, as is default on the baseline model. This keeps the total hidden size identical between models and ensures a fair comparison.  %\\
