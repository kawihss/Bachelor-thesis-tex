...
\section{Research Questions and Experiments}
    ...
    %\end{itemize}
    %\vspace{-8pt}
    %If \emph{yes}, we investigate encodings of domain knowledge into learned models: \ws{ TODO it is not the best to have conditional research questions. Repharse them so that you do not need the "if yes".} \me{I will leave this for now to wait for tims comment. Otherwise, it can simply be left out}
    %\begin{itemize}
    %\vspace{-8pt}
      \item \textbf{RQ3:} 
...
%\todo{hypothese dass wir vermuten pro domäne besser, dann das exp 2.}
        ...
        Models were trained and evaluated on the full dataset. This experiment served as the overall comparison across models and model families and aims to answer RQ1.
...
A fixed random seed was used across all models and experiments to support reproducibility (see Table~\ref{tab:hpo-config} in Appendix~\ref{appx:B}). Note, however, that this seed does not cover the  data augmentation applied during preprocessing, so all experiments must be run on the same, already-generated Parquet dataset rather than regenerating preprocessing independently.
 Each model received the  normalised feature set 
    as a context window and output displacement increments $(\Delta x, \Delta y)$. The kinematic models additionally received x and y coordinates as an input, an input we omitted for the deep learning models as per Section \ref{subsec:featureengineering}. %to avoid learning specific trajectories instead of vessel dynamics. The kinematic models are incapable of "overfitting " to a specific location, so raw coords are fine
    The predicted displacements were cumulatively summed and offset by the last known position to reconstruct absolute trajectories before any metric was applied. 
     \ac{HPO} was performed exclusively on the validation set. The final model and parameters were then evaluated on the test split without retraining.
    

      \subsection{Train-Val-Test Split}
            While formal definitions of the \ac{HPO} and \ac{CASH} problem typically use $k$-fold cross-validation to guide the search \parencite{AutoMLHoos, Feurer2015}, this is mainly designed for traditional machine learning on smaller datasets. For deep learning on our large \ac{AIS} trajectory datasets, we instead used a fixed split between training, validation, and test dataset. In particular, we used a stratified 70:15:15 split, as specified in Section \ref{subsubsec:stratifiedtvtsplit}.  %\\ % for two reasons:
    ...
    \subsection{Metrics}
    All trajectory-based metrics were computed from the Euclidean distance between predicted and ground-truth positions. The evaluation was performed over all files in the considered evaluation split and included all predicted time steps, not only the final one, which is separately reflected by the \acf{FDE}. Consistent with common trajectory prediction practice, we additionally reported the \acf{ADE} over all predicted time steps and samples \parencite{donandt2022shortterm, articlebilstmsus}.
    
    In \ac{HPO}, we optimised the \acf{RMSE} of the predicted $x$ and $y$ coordinates. \ac{RMSE} is a standard metric for regression-based prediction tasks and is also used in vessel trajectory prediction by \textcite{articlebilstmsus} and \textcite{donandt2022shortterm} (called $ATE$ in the latter). %$ATE^2$ ist MSE
    ...
    %if really necessarry, i could add an appendix entry, but the explanation with the ranking should siffice imo. Reader is probably already familiar with anova and Friedman was used in many AIM . 
    ...
    Stochastic filters and deep learning architectures depend on hyperparameter configurations that cannot be easily derived from training data and are impractical to tune manually at scale.
    Manual tuning risks a biased evaluation, where a model might underperform simply due to a suboptimal configuration rather than an inherent lack of predictive capability.
    To guarantee a fair comparison, each model was optimised independently using \ac{AutoML} in the form of \acf{HPO}. The scope was deliberately restricted to \acs{HPO} rather than the full \ac{CASH} formulation, since domain-specific meta-learning data for a warm start was unavailable. The set of candidate model classes (kinematic models, \acp{KF}, \ac{LSTM} networks, and foundation models) was chosen manually, while their respective hyperparameters were explored automatically.
    
       % \paragraph{Optimisation Methodology.}

        We centralised the \ac{HPO} for all models. We used the Optuna framework \parencite{optuna_2019}, which supports flexible definition of search spaces, including continuous, integer, and categorical parameters. Optuna was selected because it is easy to set up locally on both Windows and Linux systems, and can be expanded to be used on a cluster.
        For the Deep Learning models, the hyperparameter search was driven by the \acf{TPE}.         
         ...
         To further speed up convergence, we initialised the hyperparameters of the stochastic filters with values found via \ac{HPO} on a small subset of the data. % \me{these werre seeded at random, but i didnt save the seed and didnt save the subset unfortunately}
        ...
        study are biased towards values that are important early on in the convergence, rather than final validation loss, since most trials do not run to full completion.
         ...
         Optuna keeps track of intermediate results, enabling us to analyse the learning dynamics after an optimisation run.
          A complete specification of the hyperparameter optimisation methodology and the configurations for all experiments is detailed in Table \ref{tab:hpo-config} in Appendix \ref{appx:B}.
          Furthermore, the corresponding hyperparameter search spaces for each model are defined within the respective methodology sections (\ref{sec:method-cv} to \ref{sec:method-ctrv-ekf}) and are additionally summarised in Table \ref{tab:model_hyperparameters} in Appendix \ref{appx:B}. %and \ref{sec:method-lstm}
        ...
        \begin{itemize}
            ...
            \item \textbf{Foundation Models:} Chronos is designed as a pure Zero-Shot model and requires no \ac{HPO}. For TiREX, we optimised the size of the input window using the same 1-D space as the kinematic models. The optimisation is required, because the TiREX does not implement any attention mechanisms that can weight specific timestamps.
        ...
        \end{itemize}
           ...
           Within the optimisation process of our models, we treated the input feature subset as a categorical hyperparameter. Rather than relying on manual feature ablation, the selection of which \ac{AIS} signals to include was integrated directly into the search space. This reflects prior research showing that optimal feature subsets and model hyperparameters are interdependent, as the best configuration for one inherently depends on the other \parencite{Feurer2015}.
...
\begin{table}[t]
    ...
    \texttt{feature\_set}   & Categorical & $\{\{\mathrm{d}x_{\mathrm{norm}},\, \mathrm{d}y_{\mathrm{norm}}\} \cup S \mid S \in \mathcal{P}(\mathcal{A})\}$ \\
    \bottomrule
    \end{tabular}
    \\[1ex]
    \small \textit{Note:} $\mathcal{A} = \{\{\sin(\mathrm{cog})_{\mathrm{norm}}, \cos(\mathrm{cog})_{\mathrm{norm}}\}, \mathrm{sog}_{\mathrm{norm}}, \mathrm{rot}_{\mathrm{norm}}, \mathrm{d}t_{\mathrm{norm}}\}$
    
    \caption[LSTM Hyperparameter Search Space]{Hyperparameter search space used for the \ac{LSTM} model optimisation. Continuous parameters are sampled from uniform or log-uniform distributions, categorical parameters are selected from predefined sets. For the \texttt{feature\_set}, the search space consists of all feature combinations formed by the base input $\{\mathrm{d}x_{\mathrm{norm}}, \mathrm{d}y_{\mathrm{norm}}\}$ united with any subset $S$ drawn from the power set $\mathcal{P}(\mathcal{A})$. The $\sin$ and $\cos$ components of \acf{COG} ($\sin(\mathrm{cog})_{\mathrm{norm}}$ and $\cos(\mathrm{cog})_{\mathrm{norm}}$) are treated as an indivisible pair within $\mathcal{A}$.}
    \label{tab:lstm-hpo}
\end{table}
...
\me{i moved this full section into the experimental setup. all general information is included in the methdos section}
    ...
    In this section, we describe the sequence models we have selected for vessel trajectory prediction and how they were implemented. Two categories were evaluated: foundation models applied zero-shot, and \ac{LSTM} models trained on the full dataset.      
        ...
        \begin{enumerate}
            \item Univariate channels: To predict $dx, dy$, we fed the model with the normalised $dx\_norm$ and $dy\_norm$ channels, which the model treats independently.
            \item Zero-Shot Evaluation: We used the model as is. Because no attention mechanism is implemented, we optimised only the input length. Due to the univariate character, feature selection was not a factor in our optimisation.
    ...
    In contrast to the kinematic models and filters, which encode an explicit motion
    ...
    procedure.
        ...
        The model follows a \ac{Seq2Seq} encoder--decoder design
        (Figure~\ref{fig:mylstm}): a stacked \ac{LSTM} encoder
        processes the context window and compresses it into a hidden state, which
        seeds the decoder.
        The decoder then predicts all future displacements simultaneously. We picked the linear prediction instead of an autoregressive decoder, due to the faster compute time.
        ...
        The number of layers was fixed at 2 to keep the search space reasonable. Dropout was applied between layers for regularisation. More details about the feature selection can be found in Section \ref{subsub:featureselection}. 
        \begin{figure} [h]
            \centering
            \includegraphics[width=0.85\linewidth]{figures/MyLSTM.drawio.pdf}
            \caption[Proposed LSTM Architecture]{The \acf{LSTM} Architecture implemented in this work: Two \ac{LSTM} Encoding Layers feed into a Linear Decoder. The Initial Cell and Hidden state are initialised with the Embedded Domain. The figure is self-made.}
            \label{fig:mylstm}
        \end{figure}
        %\paragraph{Feature Selection.}
%Eigentlich alles redundant mit Evaluation Framework
        ...
        In the \textit{\ac{LSTM} + domain} variant, the domain is represented as a 4-dimensional one-hot encoded vector, with one dimension per class. Instead of appending the domain vector as an input feature at every timestep, it is added to the initial hidden and cell states of both LSTM layers.
        Specifically, the last 4 nodes of the initial hidden state $h_0$ and cell state $c_0$ are set to the one-hot vector. The remaining nodes are initialised to zero, as is the default on the baseline model. This keeps the total hidden size identical between models and ensures a fair comparison.  %\\
        Encoding the domain only in the initial states is acceptable here, since we focus on short term predictions and only need to retain the contextual information for 10 timesteps. %\\
        ...
        A learned embedding layer was considered as an alternative. However, for only four domain classes, one-hot encoding is preferable as it is parameter-free, exact, and directly interpretable.
        In contrast, learned embeddings introduce additional trainable parameters and reduce interpretability, since the resulting coordinates no longer correspond directly to individual domain classes.
