...
%\todo{ Chapter 6  needs restructuring. The content is not split in sections that has the titles of Exp 1 , 2 , 3. Not the best. It should be split by research questions then answering each followed by a discussion section.}
...
%\todo{You can either try to fit the discussion in the answer itself if it does not make it too long OR You can try to following: section{Discussion} In this section, we discuss the results in more details. subsection or paragraphXXXXX (Does not have to refer directly to question 1. Just give it a meanignfultile)[11:43 Uhr]I cannot say for sure what is better because it highly depends on the amount of details and the flow}



%\me{move foundation calibration into discussion? with table?}



\begin{comment}
------------------
what I understood you want this to be like according to the feedback:

intro: We conducted 3 Experiments in which we picked .. models. the experiments differ in (..details). this chapter aims to answer the 3 main research questions.

citing RQ1
.. was answered with exp 1.. 
present metrics with descriptions, directly answer the rq1 (pure result, no interpretations)

citing RQ2
.. was answered with exp 2.. 
present metrics with descriptions, directly answer the rq2 (pure result, no interpretations)


..RQ3


discuss RQ1-RQ3 results in a merged text (expected? causes, consequences, shortcomings..)

final conclusion ( in the conclusion chapter)

OR what whould align more with your paper:
...
.. was answered with exp 1.. 
present metrics with descriptions, directly answer the rq1 (pure result, no interpretations)
discuss RQ1 results (expected? causes, consequences, shortcomings..)

citing RQ2
.. was answered with exp 2.. 
present metrics with descriptions, directly answer the rq2 (pure result, no interpretations)
discuss RQ2 results (expected? causes, consequences, shortcomings..)
...
personally i would prefer not to have a merged discussion of all experiments, I think it will be hard to make transitions as they all investigate different topics. Additionally, i have many figures and you would need to skip back ~5 pages.
in your paper you merged results and discussion (though you separated them by outlining results first). that aligns with the 2nd version i outlined here. I can do it similarly, but i would likely add paragraoh subsection titles since IRT put big emphasis on dividing those 2 in their writing guidelines
-------------
\end{comment}

%\todo{format figures to get better page breaks once the text is final}
%intro
...
The overall goal of these experiments was to provide a fair comparison \ws{of}
of deep learning models, kinematic models, and stochastic filters, \ws{as well as} a
s well as to investigate the impact of incorporating geographic domain information into model training and architecture.
\begin{comment}
 Specifically, we aimed to answer the following research questions:

\begin{itemize}
    \item \textbf{RQ1:} Given a preprocessed dataset and a systematic hyperparameter optimisation procedure, how do deep learning models, both task-specific and pretrained, kinematic baselines, and stochastic filters compare in short-term trajectory prediction of inland vessels?
    \item \textbf{RQ2:} Does domain-specific training improve short-term vessel trajectory prediction performance compared to mixed-domain training?
    \item \textbf{RQ3:} Does adding explicit domain information as one-hot encoded input improve short-term vessel trajectory prediction performance?
\end{itemize}
\end{comment}
...
The remainder of this chapter is structured around the  questions, for each of which we present the quantitative results first, along with the answers to our research questions. A Section with a more detailed discussion of results follows.

\ws{please make the links explicit. It is needed for printed versions. You can use footnotes, for example to list the full link}
...
The significant differences are illustrated as a \ac{CD} diagram in Figure~\ref{fig:cd_test}. \ws{we conducted Friedman .... then say the significant differnces are illustrated as a CD digaram in Figure xx} 
...
\begin{comment} % split up version below
\begin{table}[t]
\centering
...
\ws{In the results chapter, mention some quantitative results from the tables and the figures. It is not enough to refer to them. Things like XX is xx\% better than xx or ranks better ..} \me{done}
...
%Figure~\ref{fig:paralelcord} 
It presents a parallel coordinate plot of the parameter search with a clear band around the optimal configuration. The plot indicates that the in optimisation, a feature set that explicitly excluded the \ac{ROT} was selected.
...
\ws{`Furthermore' transition is not needed. Just mention that you sat Exp II to answer this}
...
\ws{You need to mention quantified results, not enough to refer to tables or figures} \me{done for all experiments}
\paragraph{Results.}
...
For the \ac{CV} amodel, no change in performance ($0.00$\,m difference) is observed between mixed and domain-specific training, while the \ac{CTRV} variants exhibit deviations of up to $12.55$\,m in \ac{RMSE}.
For the \ac{EKF} and \ac{LSTM}, however, substantial performance drops occur under mixed training, particularly in the channel and lock domains (Table \ref{tab:results_exp1_exp2_merged_deltas}, shaded in orange).\ws{where? You never mentioned the table or the figure}.
...
Finally, it can be observed that the harbour domain is the only environment where domain-specific training fails to produce a statistically significant improvement for the \ac{LSTM}. Further details about the statistical significance across domains can be found in ~\ref{fig:cd-diagrams-a}
...
In Table~\ref{tab:results_exp3}, the performance metrics of the standard \ac{LSTM} (first introduced in Table \ref{tab:results_exp0} and \ref{tab:results_exp1_exp2_merged_deltas}) are provided \ws{recalls?} \me{specified.  it is repeated for the reader's convenience to compare the models} 
as a benchmark for comparison, alongside the metrics of two domain-aware variants: LSTM + Domain, which was independently tuned using a separate hyperparameter optimisation study, and LSTM + Domain*, a variant using the exact same hyperparameter configuration as the baseline architecture. 
Both domain-aware models achieve lower overall error metrics than the standard \ac{LSTM}. For example, the LSTM + Domain* variant reduces the overall \ac{RMSE} from $111.36$\,m to $110.20$\,m, representing a decrease of $1.04\%$. In Figure~\ref{fig:violindomaine}, the corresponding performance distributions of both models are presented.

To ensure that the observed performance gap is robust, the baseline \ac{LSTM} and the LSTM + Domain* model were trained ten times with different random seeds. Across all ten runs, strictly lower test errors in \ac{ADE}, \ac{FDE}, and \ac{RMSE} were recorded for the LSTM + Domain* model.
Notable performance improvements are yielded by the domain-aware model, particularly in the channel and lock domains (shaded in cyan). For instance, the \ac{ADE} decreases from $32.09$\,m to $29.22$\,m in the channel domain, and from $45.41$\,m to $31.43$\,m in the lock domain.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/thesis_violin_lstm_vs_nohpo_lstm_domain_per_year.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.69\linewidth]{figures/cd_diagram_lstm_comparison.png}
...
\ws{make it as section*. It is not good to have only one numbered sub-section}

\ws{The section does not establish or examines. Rephrase. Also, given that you do not have section numbers here, it is better to no to mention `previos section'. You can start with, in what follows, we discuss the mechanisims .. }
...
\ws{Predictive Capabilities of Motion Models. The titles are usually nominalised, not verbalised. } \me{fixed}
...
%\todo{resolve appendix reference; as a resort just leave this out if the intuition is clear that circle deviate more than straight lines}
\ws{you can refer to them like this: In Appendix XX (Figures XX and XX). It needs to be explicit that these are in the appendix}
...
The different order of magnitude for $r_\text{pos}$ in different domains, and the respective value differences between \ac{KF} and \ac{EKF} allow us to conclude, that a fixed measurement noise to values commonly found in literature would not have been sufficient.
Despite the optimised tuning, the \acp{KF} fails to surpass the simple \ac{CV} model. This points to two primary conclusions regarding its limitations.
...
Therefore, while the \ac{EKF} provides a measurable baseline improvement, its fundamental handling of sudden manoeuvrers and outliers is not drastically improved, consistent with the still prevailing non-Gaussian error distribution. %(Figure \ref{fig:qqplot_ctrv}). \todo{resolve appendix reference}
...
\ws{do not use `to' refer tho them all. Also, add in Appendix XX}), indicating the foundation model predicts straight-line kinematics.
...
A visual inspection of the generated trajectories (Figures~\ref{fig:domain1}, \ref{fig:domain2}, \ref{fig:domain3}, and \ref{fig:domain4} in Appendix~\ref{chap:appendixdataplots}) explains this advantage: the \acp{LSTM} particularly excel at estimating the correct velocity during acceleration and deceleration phases.
...
Beyond the pruning strategy, the hyperparameter importance analysis demonstrates that the input feature set is the most influential parameter for model success. Details about hyperparameter importances can be found in Figure \ref{fig:lstmhpimportances} in Appendix~\ref{chap:appendixdataplots}. %\todo{resolve appendix reference}
...
When extending the model architecture to future waterways, this varying sensitivity suggests that new training domains should be defined by discovering naturally distinct motion regimes within the data. These findings confirm that domain-specific training is indeed critical for high-capacity models, thus answering the second research question.
