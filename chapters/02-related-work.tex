...
Classical kinematic models remain common; however, their simplifying assumptions limit their ability to capture complex manoeuvres.
Data-driven methods trained on \ac{AIS} data have become a promising alternative.
...
Finally, we discuss the necessity of systematic hyperparameter optimisation to enable a fair comparison between models in Section \ref{sec:hpoforvesselprediction}.

\section{Kinematic and Deep Learning Models} 
\label{sec:kinematicanddeeplearningmodels}



\paragraph{Kinematic Models and Kalman Filters.} 
...
kinematic models such as \ac{CV} or \ac{CTRV} \parencite{li2003survey}.
...
They are computationally lightweight and well suited for short-term predictions under linear conditions.
When it comes to 
%vessel dynamics, \acp{KF} were proposed only as a means to overcome the strict linearity constraint of \ac{CV} models.
vessel dynamics, the process noise covariance Q of a \ac{KF} can be tuned to allow for deviations from the assumed linear motion. However, the underlying prediction model remains linear and therefore, their useful prediction horizons remain limited ~\parencite{donandt2022shortterm}. %das zitat bezieht sich jetzt nur auf "useful prediction...
...
by linearising the state transition and measurement functions using first-order Taylor expansion, yet the linearisation still introduces approximation errors under highly nonlinear motion regimes~\parencite{kfreview}.
%Both filter types rely on an explicit, known motion model.
Both filter types are inherently model-based and require prior knowledge of the system dynamics in the form of predefined state transition equations.
These two filters cannot accurately capture vessel behaviour when the underlying dynamics depend on unobserved spatial factors %, such as shallow reefs or islands
...
With increasing \ac{AIS} data availability, data-driven models, particularly deep neural networks, have gained popularity as alternatives or complements to model-based approaches. 
They are capable of handling complex navigation situations and longer prediction horizons. 
...
Another key development was the \ac{Bi-LSTM}, which processes sequences in both forward and backward directions to better capture the full context of a trajectory segment.
\textcite{articlebilstmsus} showed that \acp{Bi-LSTM} can significantly outperform traditional forecasting models such as \ac{ARIMA} and \acp{MLP}, alongside \acp{LSTM} in complex maritime scenarios.
...
The spatial attention mechanism utilises concurrent \ac{AIS} observations from neighbouring vessels as additional input. This approach directly applies to the scenarios relevant to autonomous surface vessels.
%While this approach directly applies to , its spatial attention mechanism requires concurrent \ac{AIS} observations from multiple neighbouring vessels.

Despite these advances, existing deep learning studies remain largely limited to open-ocean or coastal settings.
\textcite{articlebilstmsus} evaluated their \ac{Bi-LSTM} model exclusively on large vessels in open coastal waters around Taiwan, where trajectories are comparatively straight and predictable.
...
%works
%that targeted short-term inland prediction but with a limited
%model training and evaluation scope restricted to a single 90-kilometre section of the Danube River, where they used 
%between 250k and 280k trajectory sequences.
Subsequent work by \textcite{legel2025} extended the attention mechanism presented by \textcite{sekhon2020} to an interaction-aware multi-vessel \ac{LSTM} explicitly adapted for the inland domain; their work is, however, limited to a 16km stretch of the Rhine. %auf nachfrage gibt es hier noch paper dazwischen der donandt gruppe. da geht es aber um die entwicklung dieser attention mechanismen, die sind nicht fous der arbeit. mein pukt den ich hier mache ist, dass alle nur begrenzt ausgewertet wurden.

In summary, existing deep learning approaches either rely on limited spatial coverage, small data\-sets, or the inputs of multiple vessels. They rarely include sys\-tem\-at\-ic \ac{HPO}. %hbox overfull shenanigans
%This thesis instead focuses on single-vessel short-term prediction on a large, inland \ac{AIS} dataset with explicit waterway domains, and jointly evaluates kinematic models, stochastic filters, and deep sequence models under a unified optimisation and preprocessing pipeline for inland data.
...
With the rising popularity of \acp{LLM}, 
foundation models have been explored in time series forecasting \parencite{timeseriesreview}. 
Time series foundation models %such as Chronos \parencite{ansari2024chronoslearninglanguagetime} and TiRex \parencite{auer2025tirexzeroshotforecastinglongtrex} 
are large forecasting models that are pretrained on massive datasets of, typically, both real and synthetic data, with tens of billions of points from different domains \parencite{timeseriesreview}.  % ciation: review has number of points and examples for synthetic and real datasets, the claim follows.
They are designed to provide universal, often zero‑shot, probabilistic forecasts, and can be adapted to new tasks with little or no fine-tuning \parencite{ansari2025chronos2}. 
...
To address the limitations of existing studies to small sample sizes and narrow geographic constraints, this thesis provides a broader and more diverse evaluation basis. To achieve this, our work used a dataset of over four million sequences across multiple measurement stations for inland waterways with explicitly defined waterway domains. 
While incorporating multi-vessel interactions is highly relevant for autonomous surface vessels, it complicates the direct comparison between learned and kinematic models, as the latter cannot natively incorporate dependencies between vessels and would require a different input space. 
Within the modelling scope of this work, we therefore assume that predictions rely exclusively on the historical \ac{AIS} data of a single vessel, meaning each target is predicted independently without relying on concurrent observations of neighbouring traffic. %this is more of a limitation, but eihter way it should be mentioned. It is the scope i was given by Tim
Finally, we incorporate the aforementioned foundation models into our evaluation framework to assess how their zero-shot capabilities compare against the other models under a unified optimisation and preprocessing pipeline.
...
Raw \ac{AIS} data contains errors such as physically implausible positions, inconsistent timestamps, and unreliable velocity readings \parencite{Zhao_Shi_Yang_2018}. Without systematic preprocessing, such artefacts propagate into learned motion representations and may corrupt model training.
To address this, \textcite{Zhao_Shi_Yang_2018} proposed a foundational framework for \ac{AIS} preprocessing.
Their framework includes multiple steps.
First, physical integrity checks are applied, where invalid data points violating fixed bounds are discarded and trajectories with fewer than 100 points are removed.  
Second, checks designed to enforce spatial integrity by removing physically implausible transitions are applied.
...
The methodology proposed in our work explicitly seeks to avoid this limitation.
...
They result in sub-optimal configurations and make robust cross-study and fair comparisons between models difficult. 
...
To address this, we apply systematic \ac{HPO} methods, following the broader \ac{AutoML} paradigm of automating model configuration, aiming to ensure a fair and reproducible benchmark across all evaluated model families.
