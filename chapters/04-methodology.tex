...
Recently, \textcite{beck2024xlstmextendedlongshortterm} introduced the \ac{x-LSTM} architecture to address bottlenecks of standard \acp{LSTM}. The primary goals of this architecture are to improve hardware scalability and to provide a larger memory capacity. To achieve this, two new \ac{LSTM} layers were introduced: First, the \ac{s-LSTM} introduces exponential gating, which allows the model to revise and update its memory more dynamically. Second, the \ac{m-LSTM} replaces the traditional vector cell state and its element-wise updates with a matrix memory updated via matrix multiplication. This %structural shift eliminates the sequential dependency of the hidden state, enabling 
...
Nevertheless, because of its scaling capabilities, the \ac{x-LSTM} builds the basis of the TiRex foundation model. % (Section \ref{subsec:foundation}).
...
Building upon this, \textcite{attentioniayn} proposed the Transformer architecture, which completely discards recurrent layers (such as \ac{RNN} or \ac{LSTM}) in favour of Self-Attention. 
...
Because Transformers parallelise so effectively, they enabled the creation of foundation models. These are massive neural networks pretrained on vast, diverse datasets, across multiple domains such as weather, finance, and sensor data, to learn universal sequence patterns before being applied to a specific task. Once trained on very general data, they can be fine-tuned or adapted to specific tasks with relatively small amounts of task-specific data \parencite{Liang_2024foundation}.

A prominent example of foundation models in time series forecasting is \textbf{Chronos} \parencite{ansari2024chronoslearninglanguagetime}, which tokenises time series values and processes them using a pure encoder architecture, or the more recent adaptation \textbf{Chronos-2} \parencite{ansari2025chronos2}, which uses group attention to maintain weights across channels and enable multivariate or covariate informed forecasting. For our vessel trajectory prediction, foundation models offer two  application paradigms, of which we only employ the former due to a lack of pre-trained models:
\begin{itemize}
    ...
    \item \textbf{Fine-Tuning (Warm Starting):} The model can be trained on our specific domain data. By initialising the training process with pretrained weights (a \enquote{warm start}) rather than random initialisation, the model requires significantly less domain data and computational time to adapt to the specific kinematics of inland vessels. %\me{we dont do that. still include the possibility?}
...
The \acf{KF} %, first introduced by \textcite{Kalman1960},
...
Additionally, it contains information about the certainty of the estimation. 
It is %a highly efficient algorithm 
frequently used in trajectory tracking and autonomous navigation \parencite{kfapplications}. The core operation of the \ac{KF}, % as described in their lecture notes,
can be summarised as a recurring two-step algorithm of prediction and correction \parencite{Kalman1960}. 
During prediction, the filter executes a pure forward simulation based on the system model and the previously estimated state. 
...
During correction, when a measurement becomes available, the measurement model is used to project the predicted state into a predicted measurement. 
...
The \ac{KF} yields optimal state estimates under the assumption of linear system dynamics and Gaussian noise.
For comparative purposes, we isolated the prediction phase in our implementation: during the context window, the filter runs normally with regular measurement corrections to stabilise its state estimate. Once the context is consumed, the filter performs predictions without corrections.
...
\end{equation}
    ...
    \begin{comment} % prettier version but no space
...
\end{equation}
\end{comment}
The diagonal covariance matrices were treated as hyperparameters. Using $\operatorname{diag}(\cdot)$ to denote a diagonal matrix, they are defined as $\mathbf{Q} = \operatorname{diag}(q_\text{pos}, q_\text{pos}, q_\text{vel}, q_\text{vel})$, $\mathbf{R} = \operatorname{diag}(r_\text{pos}, r_\text{pos})$, and $\mathbf{P}^+_0 = \operatorname{diag}(p_{0,\text{pos}}, p_{0,\text{pos}}, p_{0,\text{vel}}, p_{0,\text{vel}})$.
    ...
    Its diagonal structure assumes uncorrelated positional and velocity noise, neglecting the physical coupling that exists because velocities are derived from positional differences.
    $\mathbf{R}$ reflects the positional uncertainty of \ac{AIS}
...
with $\Delta\psi$ now depending on the current state rather than a fixed value, %, as defined by Equation~\ref{eq:ctrv-ekf-f}, 
with the linear \acs{CV} model substituted whenever $|\omega_k| < \varepsilon$.
    ...
    \ac{KF}. %\\    
    The observation matrix $\mathbf{C}$ extracts only the position. Both the transition function $f$ and the observation matrix $\mathbf{C}$ are formulated together in Equation \ref{eq:ctrv-ekf-f_and_c}.
...
The covariance matrices extend those of the \ac{KF} with the \ac{CV} by one dimension, resulting in $\mathbf{Q} = \operatorname{diag}(q_\text{pos}, q_\text{pos}, q_\text{vel}, q_\text{vel}, q_\text{rot})$, $\mathbf{R} = \operatorname{diag}(r_\text{pos}, r_\text{pos})$, and $\mathbf{P}^+_0 = \operatorname{diag}(p_{0,\text{pos}}, p_{0,\text{pos}}, p_{0,\text{vel}}, p_{0,\text{vel}}, p_{0,\text{rot}})$. 
The turn rate is initialised to $\omega_0 = 0$. The context phase is restricted to the most recent $n_\text{ctx}$ observations, making the state estimate more responsive to recent manoeuvres. All eight parameters $q_\text{pos}$, $q_\text{vel}$, $q_\text{rot}$, $r_\text{pos}$, $p_{0,\text{pos}}$, $p_{0,\text{vel}}$, $p_{0,\text{rot}}$, and $n_\text{ctx}$ are treated as hyperparameters.
